    ...
    pub fn description(&self) -> String {
        match self {
            Wallet::SingleKeyWallet(wallet) => {
                format!("Single Key Wallet \npublic key: {} \naddress: {} \nbalance: {}", hex::encode(wallet.public_key.as_slice()), wallet.address.as_str(), wallet.balance_dash_formatted())
            }
        }
    }
...
impl Encode for SingleKeyWallet {
    fn encode<E: Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
        ...
        let string_utxos = utxos.iter().map(|(outpoint, txout)| {
            (outpoint.to_string(), txout.value, txout.script_pubkey.to_string())
        }).collect::<Vec<_>>();
...
impl Decode for SingleKeyWallet {
    fn decode<D: Decoder>(decoder: &mut D) -> Result<Self, DecodeError> {
        ...
        let private_key = PrivateKey::from_slice(bytes.as_slice(), Network::Testnet).expect("expected private key");
        ...
        let utxos = string_utxos.iter().map(|(outpoint, value, script)| {
            let script = ScriptBuf::from_hex(script).map_err(|_| InsightError("Invalid scriptPubKey format from load".into())).unwrap();
            (OutPoint::from_str(outpoint).expect("expected valid outpoint"), TxOut {
                value: *value,
